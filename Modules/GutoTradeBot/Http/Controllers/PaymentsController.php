...
class PaymentsController extends MoneysController
{
    ...
    public function getUnliquidatedMenuForUsers($bot)
    {
        ...
        foreach ($senders as $sender) {
            ...
            if ($amount > 0 || $descendants_amount > 0) {
                ...
                array_push($menu, [["text" => $suscriptor->getTelegramInfo($bot, "full_name") . " " . Moneys::format($bot->ProfitsController->getUSDTtoSendWithActiveRate($amount + $descendants_amount)) . " 💵", "callback_data" => "unliquidatedpayments-{$sender->user_id}"]]);
            }
        }
        if (count($menu) > 0) {
            array_push($menu, [
    ...
    public function renderPaymentsByField($bot, $title, $field, $symbol = "=", $value = "")
    {
