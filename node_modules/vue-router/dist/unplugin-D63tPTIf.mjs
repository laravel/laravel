...
* @license MIT
*/
import { _ as formatMultilineUnion, b as ts, c as ImportsMap, g as throttle, h as mergeRouteRecordOverride, i as resolveOptions, l as appendExtensionListToPattern, m as logTree, o as diagnostics, p as joinPath, r as mergeAllExtensions, s as ESCAPED_TRAILING_SLASH_RE, u as asRoutePath, v as pad, y as toStringLiteral } from "./options-D3sDTRr-.mjs";
...
function extractDefinePageInfo(sfcCode, id) {
	...
	for (const prop of routeRecord.properties) if (prop.type === "ObjectProperty" && prop.key.type === "Identifier") if (prop.key.name === "name") if (prop.value.type !== "StringLiteral" && (prop.value.type !== "BooleanLiteral" || prop.value.value !== false)) diagnostics.VUE_ROUTER_B0004({ filename: id });
	else routeInfo.name = prop.value.value;
	else if (prop.key.name === "path") if (prop.value.type !== "StringLiteral") diagnostics.VUE_ROUTER_B0005({ filename: id });
