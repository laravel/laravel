$(function(){
    ...
    if(nameParameter !='' && nameParameter != null && nameParameter != undefined){
        $('.cont').text(userNamesArray[parseInt(nameParameter)]+'邀请你加入了“中国顶级企业家俱乐部”群聊，群聊参与人还有：马云 阿里巴巴、俞敏洪 新东方教育、朱新礼 汇源集团、马蔚华 招商银行、李东升 TCL集团、冯仑 万通控股、龙哥 “为你读诗”客户端……');
    }
	...
	function setZone(){		
		if(msgID<=15){
            ...
            if(msgID==5) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist-42 + 'px'}, ease: Linear.easeNone });
                dist += -42;
            }
            if(msgID==6) {
