$(function(){
    ...
    function initPageMotion(){
        ...
        $(".main").fadeIn(300,function(){
            setTimeout(function(){
                $('.longpage').show();
                document.title='企业家与演艺明星跨界俱乐部（50）';
                //setTimeout(function(){
    ...
    if(nameParameter !='' && nameParameter != null && nameParameter != undefined){
        $('.cont').text(userNamesArray[parseInt(nameParameter)]+'邀请你加入了“企业家与演艺明星跨界俱乐部”群聊，群聊参与人还有：马云 阿里巴巴、刘德华 香港明星、俞敏洪 新东方教育、冯巩 相声演员、康辉 新闻主播、龙哥 “为你读诗”客户端……');
    }

    /*wxdata['title'] = _userName+'邀请你加入群聊';
    wxdata['desc'] = _userName+'邀请你加入了“企业家与演艺明星跨界俱乐部”群聊，进入查看详情。'
	...
	function setZone(){		
		if(msgID<=13){
			if(msgID==4) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': -233 + 'px'}, ease: Linear.easeNone });
                dist += -233;
            }
            if(msgID==5) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist-98 + 'px'}, ease: Linear.easeNone });
                dist += -98;
            }
            if(msgID==6) {
