$(function(){
    ...
    var userNamesArray = ['刘德华','冯巩','筷子兄弟'];
    ...
    if(nameParameter !='' && nameParameter != null && nameParameter != undefined){
        $('.cont').text(userNamesArray[parseInt(nameParameter)]+'邀请你加入了“演艺明星高端俱乐部”群聊，群聊参与人还有：冯巩 相声演员、康辉 新闻主播、尼格买提 主持人、筷子兄弟 演艺组合、于魁智 京剧演员、龙哥 “为你读诗”客户端……');
    }

   /* wxdata['title'] = _userName+'邀请你加入群聊';
    wxdata['desc'] = _userName+'邀请你加入了“演艺明星高端俱乐部”群聊，进入查看详情。'
	...
	function setZone(){		
		if(msgID<=13){
			if(msgID==4) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': -220 + 'px'}, ease: Linear.easeNone });
                dist += -220;
            }
            if(msgID==5) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist-12 + 'px'}, ease: Linear.easeNone });
                dist += -12;
            }
            if(msgID==6) {
