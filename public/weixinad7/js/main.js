$(function(){
    ...
    if(nameParameter !='' && nameParameter != null && nameParameter != undefined){
        $('.cont').text(userNamesArray[parseInt(nameParameter)]+'邀请你加入了“中国顶级企业家俱乐部”群聊，群聊参与人还有：柳传志 联想控股、马云 阿里巴巴、俞敏洪 新东方教育、冯仑 万通控股……');
    }

    /*wxdata['title'] = _userName+'邀请你加入群聊';
    wxdata['desc'] = _userName+'邀请你加入了“中国顶级企业家俱乐部”群聊，进入查看详情。'
	...
	function setZone(){		
		if(msgID<=10){
			if(msgID==4) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': -200 + 'px'}, ease: Linear.easeNone });
                dist += -200;
            }
            if(msgID==5) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist + 'px'}, ease: Linear.easeNone });
                dist += 0;
            }
            if(msgID==6) {
            ...
            if(msgID==7) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist-176 + 'px'}, ease: Linear.easeNone });
                dist += -176;
            }
            if(msgID==8) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist-233 + 'px'}, ease: Linear.easeNone });
			...
			if(msgID==9) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist-115 + 'px'}, ease: Linear.easeNone });
                dist += -115;
            }
            if(msgID==10) {
                TweenMax.to($(".longpage>div").not($('.di')), .5, { css: {'top': dist-233 + 'px'}, ease: Linear.easeNone });
